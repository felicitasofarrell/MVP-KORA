...
const cards = [
  { 
    ...
    title: "Plataformas", 
    cat: "plataformas", 
    desc: "Listado y accesos a las principales plataformas de la empresa."
  },
  { 
    key: "funciones", 
    title: "Funciones CX", 
    cat: "funciones", 
    desc: "Resumen de las tareas principales del rol de Customer Experience."
  },
  { 
    key: "ftp", 
    title: "FTP / SFG / STA", 
    cat: "ftp", 
    desc: "Guía rápida para conectar y transferir archivos por FTP, SFG y STA."
  },
  { 
    key: "plantillas-excel", 
    title: "Plantillas Excel", 
    cat: "plantillas", 
    desc: "Plantillas prearmadas para agilizar reportes y cálculos."
  },
  { 
    key: "procesadoras", 
    title: "Procesadoras de Pago", 
    cat: "procesadoras", 
    desc: "Información sobre conexiones y gestión con las procesadoras de pago."
  },
  { 
    key: "dicc", 
    title: "Diccionario Contable", 
    cat: "diccionario", 
    desc: "Glosario con los principales términos contables usados en la empresa."
  },
  { 
    key: "mails", 
    title: "Plantilla de Mails", 
    cat: "mails", 
    desc: "Ejemplos y modelos de mails listos para enviar a clientes o partners."
  },
  { 
    key: "otros", 
    title: "Otros", 
    cat: "otros", 
    desc: "Recursos adicionales."
  },
];
...
export default function Second() {
  ...
  // Navegar al chat con parámetros (tuya)
  const goToChat = (cat) => {
 ...
 // 🔹 Navegar al chat con el seed coreografiado (abre Plantillas Excel)
const goToChatWithSeed = () => {
  const roleParam = rol.toLowerCase().replaceAll(" ", "_");
  const seed = encodeURIComponent(DEFAULT_SEED);
  const seedImg = SeedImg ? encodeURIComponent(SeedImg) : ""; // opcional
  // cat=plantillas + hideWelcome=1; //para que NO muestre el saludo por defecto
  navigate(
  ...
  // 🔹 Arranca la coreografía con modal a pantalla completa
  function startFollow() {
    ...
    // Mensaje al usuario para que cambie de app
    // (si preferís, quitá el alert)
    // alert("Ahora cambiá a otra app. Tenés 10 segundos 😉");
    ...
    timerRef.current = setInterval(() => {
      setCountdown((s) => {
        if (s <= 1) {
          ...
          setPhase("return"); // cambia contenido del modal
  ...
  // 🔹 Cancelar / finalizar seguimiento
  function stopFollow() {
    // Si terminó el conteo y querés que “Dejá de seguirme” también navegue:
    if (finishedCountdown) {
      cleanupFollow();
      goToChatWithSeed();
      return;
    }
    // Si no terminó, solo cancelar
    cleanupFollow();
  }
    ...
    <div className={`sec-root ${modalOpen ? "blurred" : ""}`}>
      ...
      <section className="sec-hero">
        ...
        <div className="hero-actions">
          <button className="cta" onClick={() => goToChat("all")}>
            Chatear
          </button>
          <input
            type="text"
          ...
          <div className="modal-card">
            ...
            {phase === "return" && (
              <>
                ...
                <p className="modal-sub">
                  Ya capturé el contexto de la demo. Abrí el chat para ver sugerencias.
                </p>
                <div className="modal-actions">
